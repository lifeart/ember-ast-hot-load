...
async function testHotReloadChangedUrl(options, expectedUrl) {
  ...
  reloader.run();
  await fileDidChangeHandler({
    filePath: '/components/path/to/my-component.js',
  });
...
it('requests the correct change URL for the default live reload options', async () => {
  await testHotReloadChangedUrl(
    {},
    'http://localhost/changed?files=/components/path/to/my-component.js'
  );
});
...
it('requests the correct change URL when the live reload prefix is /', async () => {
  ...
  await testHotReloadChangedUrl(
    options,
    'http://my-host.test:36629/changed?files=/components/path/to/my-component.js'
  );
});
...
it('requests the correct change URL when the live reload prefix ends with a /', async () => {
  ...
  await testHotReloadChangedUrl(
    options,
    'http://my-host.test:1234/this/is/my/path/changed?files=/components/path/to/my-component.js'
  );
});
...
it('requests the correct change URL when the live reload server uses SSL', async () => {
  ...
  await testHotReloadChangedUrl(
    options,
    'https://my-host.test:1234/changed?files=/components/path/to/my-component.js'
  );
});
