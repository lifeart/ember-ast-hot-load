...
it('can lookup file in path using findEntypointFile', () => {
  ...
  assert( middleware.findEntypointFile('hot-load-middleware.js', __dirname), __dirname + path.sep + 'hot-load-middleware.js');
  assert( middleware.findEntypointFile('hot-load', __dirname), __dirname + path.sep + 'hot-load-middleware.js');

  const nestedPath =  path.join(__dirname, '../', 'addon');
  assert( middleware.findEntypointFile('components/hot-content', nestedPath), nestedPath + path.sep + ['components', 'hot-content.js'].join(path.sep));
});
