...
it('handle angleBracket components', () => {
    assert(
        `<FooBar />`, `{{#let (component (hot-load "FooBar" this "FooBar" "FooBar") hotReloadCUSTOMhlContext=this hotReloadCUSTOMName="FooBar" hotReloadCUSTOMhlProperty="FooBar" hotReloadCUSTOMHasParams=true hotReloadCUSTOMHasHash=false) as |HotLoadFooBarupslsp6ldfm|}}<HotLoadFooBarupslsp6ldfm></HotLoadFooBarupslsp6ldfm>{{/let}}`
    )
});
...
it('handle complex contexted angleBracket components', () => {
    assert(
        ...
        `{{#let (component (hot-load "FooBar" this "FooBar" "FooBar") hotReloadCUSTOMhlContext=this hotReloadCUSTOMName="FooBar" hotReloadCUSTOMhlProperty="FooBar" hotReloadCUSTOMHasParams=true hotReloadCUSTOMHasHash=false) as |HotLoadFooBarupslsp6ldfm|}}<HotLoadFooBarupslsp6ldfm></HotLoadFooBarupslsp6ldfm>{{/let}}
...
it('can handle mixed contexted components', () => {
  ...
  {{/let}}`,
  `{{#let (component (hot-load "foobar" this "foobar" "foobar") hotReloadCUSTOMhlContext=this hotReloadCUSTOMName="foobar" hotReloadCUSTOMhlProperty="foobar" hotReloadCUSTOMHasParams=true hotReloadCUSTOMHasHash=false) as |FooBar|}}    <FooBar></FooBar>
...
it('skip ...attributed components', ()=>{
    assert(`<FooBar ...attributes />`, `<FooBar...attributes></FooBar>`);
    assert(`<FooBar name="foo" ...attributes />`, `<FooBar name="foo"...attributes></FooBar>`);
...
it('skip attribute bindings, looking like components', ()=>{
    assert('<div name={{foo-bar}} />', '<div name={{foo-bar}}></div>');
});

it('skip element-modifiers', ()=>{
    assert('<div {{foo-bar}} />', '<div {{foo-bar}}></div>');
    assert('<div {{foo-bar (concat "a" "foo-bar")}} />', '<div {{foo-bar (concat "a" "foo-bar")}}></div>');
});

it('skip concat statements', ()=>{
    assert('{{concat (mod-by foo bar)}}', '{{concat (mod-by foo bar)}}');
    assert('{{foo (mod-by foo bar)}}', '{{foo (mod-by foo bar)}}');
    assert('<div attr="test {{class-name}}" />', '<div attr="test {{class-name}}"></div>');
});
