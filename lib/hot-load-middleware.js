        ...
        components.forEach((componentName) => {
          if (normalizedName.includes(componentName)) {
            if (
              ...
              normalizedName.endsWith('/' + componentName + '/' + 'index') ||
...
function findEntypointFile(
  ...
  if (lookupFolder.charAt(lookupFolder.length - 1) !== path.sep) {
    lookupFolder = lookupFolder + path.sep;
  }
  let filePathParts = [];
  if (fileName.includes('/')) {
    filePathParts = fileName.split('/');
    fileName = filePathParts.pop();
    lookupFolder = lookupFolder + filePathParts.join(path.sep) + path.sep;
  }
...
function onFileRequested(req, res) {
  ...
  let filePath = ['dist', 'assets', appFileName].join(path.sep);
  let vendorPath = ['dist', 'assets', 'vendor.js'].join(path.sep);
  ...
  fs.readFile(filePath, 'utf8', function (err, data) {
    ...
    var processedResult = extractFileDataToResult(
      data,
      queryFile,
    ...
    if (!fs.existsSync(vendorPath)) {
      ...
      fs.readFile(vendorPath, 'utf8', function (errs, vendorData) {
        if (errs) {
          if (appItems.length) {
            res.send(generateFileFromArray(appItems));
          } else {
        ...
        var processedVendorResult = extractFileDataToResult(
          vendorData,
          queryFile,
          req.query.components
        );
